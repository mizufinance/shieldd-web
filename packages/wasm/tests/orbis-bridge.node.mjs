...
const pkg = {
  transfer_epk: [
    ...Buffer.from('bc5ffbb7f13ea8eeb0f0364910fb6331945996a1d2389d6ac48ded74b3e2a503', 'hex'),
  ],
...
test('matches the shipped admin cross-language seed-binding vector', async () => {
  ...
  assert.equal(
    Buffer.from(result.transfer_seed_binding).toString('hex'),
...
test('rejects malformed lengths and noncanonical fields', async () => {
  ...
  await assert.rejects(
    upgradeOrbisAuditPackage({ ...pkg, transfer_c2: Array(32).fill(255) }),
    /canonical/,
  );
});
