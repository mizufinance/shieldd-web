...
export type OrbisAuditPackage = Omit<LegacyOrbisAuditPackage, 'transfer_shared_fq'> & {
  transfer_seed_binding: number[];
};
...
export const upgradeOrbisAuditPackage = async (
  pkg: LegacyOrbisAuditPackage,
): Promise<OrbisAuditPackage> => {
  ...
  const transfer_seed_binding = Array.from(
